...
pca_db_names = {name: f'{name}_pca_tsne_cache.db' for name in datasets}
json_files = {name: f'{name}_tsne_embeddings.json' for name in datasets}
# Create separate SQLite databases for each dataset
for db_name in embedding_db_names.values():
    create_database(db_name)
for db_name in pca_db_names.values():
    create_database(db_name)
...
for dataset_name, dataset in datasets.items():
    selected_conversations = reservoir_sampling(dataset['train'], 1000)
...
# Save the PCA model
#joblib.dump(pca, 'pca_model_both.pkl')
...
for dataset_name, embedding_data in embedding_data_by_dataset.items():
    for data in embedding_data:
        embedding_2d = pca.transform([data['e']])[0]
        ...
        #insert_or_update(pca_db_names[dataset_name], data['i'], '', data['e'])
...
print("Embedding processing, PCA projection, and saving completed.")
