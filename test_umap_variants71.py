...
def process_english_data():
    ...
    umap = ParametricUMAP(n_components=2, n_neighbors=30, repulsion_strength=0.001, metric='cosine')
    #umap.n_training_epochs = 0.5
    umap_embeddings = umap.fit_transform(embeddings)

    # Save UMAP model
    os.makedirs('umap_model/debugr0001', exist_ok=True)
    ...
    umap.save('umap_model/debugr0001')
    ...
    os.makedirs('static/debugr0001', exist_ok=True)
    ...
    for dataset_name in ['wildchat', 'lmsyschat']:
        ...
        # Save full JSON
        with open(f'static/debugr0001/{dataset_name}_embeddings_all.json', 'w') as f:
            json.dump(json_data, f)
        
        # Save and gzip subsampled JSON
        #subsampled_json_data = random.sample(json_data, min(1500, len(json_data)))

        subsampled_json_path = f'static/debugr0001/{dataset_name}_embeddings.json'
