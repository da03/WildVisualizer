...
def process_english_data():
    ...
    # Save UMAP model
    os.makedirs('umap_model/debugr01', exist_ok=True)
    ...
    os.makedirs('static/debugr01', exist_ok=True)
    ...
    for dataset_name in ['wildchat', 'lmsyschat']:
        ...
        # Save full JSON
        with open(f'static/debugr01/{dataset_name}_embeddings_all.json', 'w') as f:
            json.dump(json_data, f)
        
        # Save and gzip subsampled JSON
        #subsampled_json_data = random.sample(json_data, min(1500, len(json_data)))

        subsampled_json_path = f'static/debugr01/{dataset_name}_embeddings.json'
