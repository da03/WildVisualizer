...
def process_english_data():
    ...
    #umap = ParametricUMAP(n_components=2, n_neighbors=30, repulsion_strength=0.5, metric='cosine')
    #umap = ParametricUMAP(n_components=2, n_neighbors=30, repulsion_strength=0.2, metric='cosine')
    #umap = ParametricUMAP(n_components=2, n_neighbors=30, repulsion_strength=0.1, metric='cosine')
    #umap = ParametricUMAP(n_components=2, n_neighbors=50, repulsion_strength=0.2, metric='cosine')
    #umap = ParametricUMAP(n_components=2, n_neighbors=30, spread=0.5, metric='cosine')
    #umap = ParametricUMAP(n_components=2, n_neighbors=30, spread=0.2, metric='cosine')
    umap = ParametricUMAP(n_components=2, n_neighbors=30, spread=0.1, metric='cosine')
    #umap.n_training_epochs = 0.5
    umap_embeddings = umap.fit_transform(embeddings)

    # Save UMAP model
    os.makedirs('umap_model/debugs01', exist_ok=True)
    ...
    os.makedirs('static/debugs01', exist_ok=True)
    ...
    for dataset_name in ['wildchat', 'lmsyschat']:
        ...
        # Save full JSON
        with open(f'static/debugs01/{dataset_name}_embeddings_all.json', 'w') as f:
            json.dump(json_data, f)
        
        # Save and gzip subsampled JSON
        #subsampled_json_data = random.sample(json_data, min(1500, len(json_data)))

        subsampled_json_path = f'static/debugs01/{dataset_name}_embeddings.json'
